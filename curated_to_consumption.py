...
from snowflake.snowpark.types import StructType, StringType, StructField,LongType,DecimalType, TimestampType
...
def create_date_dim(all_sales_df, session)->None:
    ...
    # Create a pandas DataFrame for date dimensions
    ...
    date_dim_df = session.create_dataframe(data = date_dim)
    ...
    date_dim_df = date_dim_df.selectExpr('\
                                   ...
                                   ')
    date_dim_df.show()
    date_dim_df = date_dim_df.drop('"DAY_COUNTER"')
    date_dim_df.show()

    insert_cnt = int(date_dim_df.count())
    if insert_cnt > 0:
        date_dim_df.write.saveAsTable("sales_dwh.consumption.date_dim", mode = "append")
        print("save operation completed")
    else:
        print("no saving is done")


def create_region_dim(all_sales_df , session)->None:
    region_dim_df = all_sales_df.groupBy(col("Country"), col("Region")).count()
    region_dim_df.show()
    region_dim_df = region_dim_df.withColumn("isActive", lit('Y'))
    region_dim_df.show()
    region_dim_df = region_dim_df.selectExpr("sales_dwh.consumption.region_dim_seq.nextval as reg_id_pk","Country", "Region", "isActive")
...
def create_promocode_dim(all_sales_df,session)-> None:
    ...
    promo_code_dim_df = promo_code_dim_df.withColumnRenamed("promotion_code", "promo_code")
    promo_code_dim_df.show(10)


    #fetch existing product dim records.
    existing_promo_code_dim_df = session.sql("select promo_code, country, region from sales_dwh.consumption.promo_code_dim")
    existing_promo_code_dim_df.show()
    #
    # promo_code_dim_df = promo_code_dim_df.join(existing_promo_code_dim_df,["promo_code", "country", "region"],join_type='leftanti')
    #
    #
    promo_code_dim_df = promo_code_dim_df.selectExpr("sales_dwh.consumption.promo_code_dim_seq.nextval as promo_code_id_pk","promo_code", "country","region","isActive")
    promo_code_dim_df.show()
    #
...
def create_customer_dim(all_sales_df, session) -> None:
    ...
    #region_dim_df.write.save_as_table('sales_dwh.consumption.region_dim',mode="append")
    # customer_dim_df = customer_dim_df.withColumnRenamed("conctact_no", "contact_no")
...
def create_product_dim(all_sales_df,session)-> None:
    ...
    #
    # #fetch existing product dim records.
    existing_product_dim_df = session.sql("select mobile_key, Model, Brand, Color, Memory, isactive from sales_dwh.consumption.product_dim")
    # existing_product_dim_df.show()
    existing_product_dim_df.count()
    #
    product_dim_df = product_dim_df.join(existing_product_dim_df,["mobile_key", "Brand", "Color", "Memory", "IsActive"],join_type='leftanti')
    # #
...
def main():
    ...
    # product_dim_df.show()
    promo_code_dim_df = session.sql("select promo_code_id_pk,promo_code,country, region from sales_dwh.consumption.PROMO_CODE_DIM")
    # promo_code_dim_df.show()
    ...
    all_sales_df = all_sales_df.withColumnRenamed("promotion_code", "promo_code")
    all_sales_df = all_sales_df.join(promo_code_dim_df, ["promo_code","country", "region"],join_type='inner')
