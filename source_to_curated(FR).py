...
def main():
    ...
    # paid_sales_df.show()s

    shipped_sales_df = filter_dataset(paid_sales_df,'SHIPPING_STATUS', 'Delivered')
    # shipped_sales_df.show()

    country_sales_df = shipped_sales_df.withColumn('Country', lit('FR')).withColumn('Region', lit('EU'))
    ...
    unique_orders = (sales_with_forex_df
                     .withColumn('order_rank', rank().over(Window.partitionBy(col('order_dt'))
                    .order_by(col('_metadata_last_modified').desc())))
    ...
    # final_sales_df.show()
    final_sales_df.write.saveAsTable("sales_dwh.curated.fr_sales_order", mode = "append")
